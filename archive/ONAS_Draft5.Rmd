...
# Get well level data
np_levels <- left_join(np, ll, by = "ID") %>% 
  mutate(NE = -1*Value, ID = as.factor(ID))

# ggplot
figure <- np_levels %>% highlight_key(~ID) %>% 
...
### Niveles Piezométricos (Tabla)

```{r}
# Clean levels table
np_levels2 <- np_levels %>% transmute(ID = ID,
                         X = East.x,
                         Y = North.x,
                         Z = Elev.x,
                         Prof. = Prof.x,
                         ...
                         Distrito = Distrito.x,
                         Corregimiento = Corregimiento.x,
                         Fecha = Date,
                         Nivel = NE)

# Create interactive datatable
datatable(np_levels2, 
          class = "default nowrap compact hover dt-head-center",
          rownames = FALSE, 
          filter = "top", 
          style = "default",
