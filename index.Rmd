...
output$MyMap <- renderLeaflet({
  ...
  #   # p_all <- lapply(wdata$ID, hydrograph, df=wdata, htype="gw")
  #   points <- wsp_ll
  # }
  # else if (input$pointTypeMap == "Rain Gages") {
  #   # p_all <- lapply(rgdata_monthly$ID, hydrograph, df=rgdata_monthly, htype="pp")
  #   points <- rgsp_ll
  # }
  leaflet(wsp_ll) %>% 
    addTiles(group = "OSM") %>%
    ...
    addMarkers(label = ~ID, layerId = ~ID, group = "markers", popup = ~New_Name) %>% 
    addLayersControl(baseGroups = c("OSM", "Toner Lite", "ESRI Imagery", "ESRI Relief"),
                     options = layersControlOptions(collapsed = TRUE),
                     position = "bottomleft") %>% 
    setView(lng = -80.2, lat = 7.9, zoom = 10.4)
})
...
output$NewMap <- renderLeaflet({
  ...
  leaflet(p, options = list(zoomControl = FALSE)) %>%
    addTiles(group = "OSM") %>%
    addMarkers(label = ~ID, layerId = ~ID, popup = ~New_Name)
})
...
observe({
  ...
  isolate({
    img <- hydrograph(id=event$id, df=wdata, htype="gw")
    leafletProxy("MyMap") %>% addPopups(popup = popupGraph(img, width = 500, height = 400), group = "markers")
  })
})

popup = popupGraph(list(img), width = 500, height = 400)
```
