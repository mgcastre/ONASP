...
selectInput(inputId = "mw", 
            ...
            choices = c("All", as.character(wells$ID)),
...
renderPlotly({
  if (input$mw == "All") {
    fig <- wdata %>% 
      group_by(ID) %>% 
      plot_ly(x = ~Date, y = ~DTW, mode = "lines+markers", color = ~ID, colors="Dark2") %>% 
      layout(yaxis = list(range = c(-22,2)))
  } else {
    fig <- wdata %>% 
      group_by(ID) %>%
      filter(ID == input$mw) %>% 
    ...
    fig %>% 
      layout(title = FALSE, plot_bgcolor = "#DCDCDC",
