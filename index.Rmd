...
observeEvent(input$pointType, {
  if (input$pointType == "Monitoring Well") {
...
output$mymap <- renderLeaflet({
  if (input$mw == "All") {
    p <- subset(wsp_ll, ID %in% unique(wdata$ID))
  }
  else {
    p <- subset(wsp_ll, ID %in% input$mw)
  }
    ...
    fig %>% 
      layout(showlegend = FALSE,
             ...
             xaxis = list(title = "Date", 
                          ...
                          zerolinecolor = "#ffff",
                          gridcolor = "ffff",
                          range = c(as.Date("2015-05-01"), as.Date("2020-04-30")),
